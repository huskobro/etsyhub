 ...
 *
 * Drift #5 (2026-04-30): riskFlags item alan adı `type` → `kind` (KIE strict
 * JSON schema reserved-word çakışması). Master prompt LLM'e `kind` döndürmesini
 * söyler — provider Zod schema (output-schema.ts) ile birebir uyumlu.
 * Bu davranış değişikliği olduğu için VERSION bump'lı: v1.0 → v1.1.
 *
 * Phase 7+ carry-forward: review-prompt-admin-screen — admin/master prompt UI.
 */

export const REVIEW_PROMPT_VERSION = "v1.1";
...

{
  ...
  "riskFlags": [{ "kind": <8 sabit kind'tan biri>, "confidence": 0-1, "reason": kısa açıklama }],
  "summary": kısa cümle
}
