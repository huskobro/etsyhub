  ...
  return (
    <main className="flex min-h-screen flex-col bg-white">
      <header className="border-b border-border px-6 py-4">
        <h1 className="text-xl font-semibold text-text">Listings</h1>
        <p className="mt-1 text-sm text-text-muted">
          Listing drafts produced from mockup packs and their Etsy submission
          status.
        </p>
      </header>
        ...
        {!isLoading && !error && listings && listings.length === 0 && (
          <div className="rounded-md border border-border bg-surface-2 p-6 text-center">
            <p className="text-sm text-text-muted">
              {activeStatus
                ? `No listings in ${LISTING_STATUS_LABELS[activeStatus]} status.`
                : "No listings yet."}
            </p>
            {!activeStatus && (
              <p className="mt-2 text-xs text-text-subtle">
                Complete a pack in Mockup Studio to create a listing draft.
              </p>
            )}
          </div>
        )}
            ...
            {listings.map((listing) => (
              ...
              >
                ...
                <Link
                  ...
                  aria-label={`${listing.title ?? "Untitled draft"} — open details`}
                ...
                >
                  {listing.coverThumbnailUrl ? (
                    ...
                    <img
                      ...
                      alt={`${listing.title ?? "Untitled draft"} cover preview`}
