...
/* Phase 132 — TEK shared framing model: aspect-locked container fit.
 *
 * KÖK NEDEN (Phase 131 öncesi): middle panel, right rail layout
 * thumb'ları ve zoom/navigator panel AYNI `compositionGroup` +
 * `cascadeLayoutFor` (cascade-layout.ts) kullanıyordu AMA
 * `compositionGroup`'a giren `plateDims` farklı kutu matematiğinden
 * geliyordu:
 *   - Middle: Stage `plateDimensionsFor` (viewport-aware bbox-fit)
 *   - Rail/Zoom: StageScenePreview lokal `{maxW:900*0.85, maxH:506*0.85}`
 *     SABİT base + ekstra wrapper `transform: scale(...)`
 * `compositionGroup`'taki `grp.scale = min(plateW×F/bboxW,
 * plateH×F/bboxH)` plate'in MUTLAK boyutuna bağlı. İki yüzey farklı
 * mutlak plate boyutu (+ Preview'da ikinci wrapper scale) → farklı
 * `grp.scale` → composition'ın plate-içi oranı (compFracOfPlate)
 * ayrışıyor. 16:9'da iki kutu da width-fit → fark %1.9; portrait
 * aspect'lerde (9:16, 4:5, 1:1) `min()` farklı eksene düşüyor →
 * fark %18-40 (kullanıcı: "9:16'da çok bariz").
 *
 * ÇÖZÜM: plate-box hesabı da TEK shared, container-agnostic,
 * aspect-locked helper. Her yüzey kendi CONTAINER kutusunu verir
 * (Middle: viewport-türevi; rail thumb: measured card box; zoom
 * pad: measured pad box); helper AYNI aspect-locked bbox-fit
 * algoritmasıyla plate dims döner. Sonuç: 3 yüzey de
 *   container → resolvePlateBox(aspect, cW, cH) → plateDims
 *   → compositionGroup(items, plateW, plateH) → scale + bbox
 * AYNI zincirden geçer. Plate aspect aynıysa (frameAspect SHARED
 * state) compFracOfPlate plate MUTLAK boyutundan BAĞIMSIZ birebir
 * (aspect-locked: plateW/plateH oranı sabit → min() hep aynı eksen
 * → compFracOfPlate sabit). "Tek görsel mantık, çoklu görünüm."
 *
 * future-proof: aspectRatio parametre (FRAME_ASPECT_CONFIG.ratio
 * veya ileride custom resolution w/h). Hardcoded aspect YOK,
 * container parametre → custom resolution / serbest width-height
 * geldiğinde sistem yeniden ayrışmaz (aspect-agnostic +
 * container-agnostic + layout-agnostic). */
export interface PlateBoxOpts {
  /** Plate container'ın ne kadarını kaplar (stage padding payı).
   *  Default ortak; her yüzey gerekirse override (ama ortak
   *  tutmak parity için tercih). */
  fillW?: number;
  fillH?: number;
  /** Plate boyut üst sınırı (çok geniş container'da dev plate
   *  olmasın — Shots.so plate mutlak ~700-900px civarı kalır).
   *  Undefined → sınırsız (preview thumb gibi küçük container'da
   *  gerek yok). */
  capW?: number;
  capH?: number;
}
...
export function resolvePlateBox(
  ...
  const fillW = opts.fillW ?? 0.9;
  const fillH = opts.fillH ?? 0.86;
  ...
  // Aspect-locked bbox-fit: hem availW hem availH'a sığ, aspect
  // DAİMA sabit (Shots.so canonical — aspect bozulmaz). Tek
  // algoritma, container ne olursa olsun.
