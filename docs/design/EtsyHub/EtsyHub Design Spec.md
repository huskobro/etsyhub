...
(Kesin kapsam: **7 bölüm · 18 artboard**. Liste bu envanteri tek doğru olarak kilitler.)
...
**5. Status title-case, badge mono ton.** Badge gramerinde iki kat: görünen metin **title-case** Türkçe (Hazır, Review, Üretiliyor, Hata, Rollout, Açık) ve **öyle kalır** — badge primitivi `text-transform: uppercase` uygulamaz. Mono hissi font ailesi + 11px boyut + 0.6 letter-spacing ile taşınır; casing title-case'tir. Badge dışı yerlerde (ör. page subtitle) aynı kelime aynı title-case okunur. Onaylı karar: **title-case kalır, uppercase yoktur.**
...
#### 3. Card (3 tip)
- **Stat card** — mono title-case label + büyük numeral + trend badge
...
#### 4. Table (admin)
- Row 48h body, 34h head. Head: surface-2 bg, mono title-case 11, tracking 0.6.
...
#### 5. Badge / Chip / Tag
- **Badge (status):** 20h, mono title-case 11, tracking 0.6, radius sm, tones: neutral/accent/success/warning/danger/info; `dot` sol 6px. `text-transform: uppercase` uygulanmaz.
...
- Sidebar 232w, surface-2 bg, accent-bar-left (2×16px bar active item'da), group headers mono title-case.
...
9-kolon tablo, head mono title-case, 1 satır selected (accent-soft). 3 chip filter, CSV export + davet CTA topbar.
