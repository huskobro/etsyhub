...
- **kie.ai z-image entegrasyonu (Task 4 / Task 16, Q6 + R17.1):** Gerçek `text-to-image` (t2i) provider; `referenceUrls` capability seviyesinde reddedilir (i2i sessiz fallback YASAK), aspectRatio 5 değere daraltılır (`1:1`/`4:3`/`3:4`/`16:9`/`9:16` — `2:3`/`3:2` runtime guard ile reddedilir). Body shape `{model:"z-image", input:{prompt, aspect_ratio}}` (`image_urls` alanı yok). Hardcoded tek-model çözüm reddedildi.
...
- **Test paketi (Task 16):** Capability mismatch (R17.1 sessiz fallback YASAK) + z-image referenceUrls reddi + aspectRatio daraltma regresyon koruması.

## Carry-Forward (named — design §10.2 ile senkron, Task 4 düzeltmesi sonrası 13 madde)

> **Düzeltme notu (2026-04-27):** İlk yazımda `kie-z-image-integration` carry-forward
> olarak listelenmişti — bu yanlıştı. `kie-z-image` Task 4 ile gerçek t2i provider
> olarak entegre edildi (`src/providers/image/kie-z-image.ts`). `caption-then-prompt-flow`
> ise z-image **entegrasyonunun** carry-forward'u değil, üstüne kurulacak UI akışının
> carry-forward'u — sebep metni revize edildi.

| # | Carry-forward isim | Sebep |
|---|---|---|
| 1 | `local-asset-resolution-fix-actions` | Çözünürlük uymayan görsel için upscale/crop aksiyonları |
| 2 | `auto-quality-detection-ocr-bg` | Arka plan + yazı/imza/logo otomatik tespiti — Phase 6 (AI Quality Review) ile gelir |
| 3 | `bulk-delete-local-assets` | Toplu silme (tek tek silme Phase 5'te) |
| 4 | `destructive-typing-confirmation` | Bulk delete + diğer riskli toplu işlemler için typing confirmation (`DELETE yaz`) |
| 5 | `export-zip-split-20mb` | ZIP paketleme + 20 MB sıralı bölme — Selection/Export ekranıyla |
| 6 | `cost-guardrails-daily-limit` | Daily/monthly limit, admin Cost Usage ekranı |
| 7 | `caption-then-prompt-flow` | t2i-with-caption UI akışı — z-image provider'ı entegre, ama "reference görseli LLM ile betimle → caption'ı z-image prompt'una çevir" akışı kullanıcı arayüzünde yok (R17.1 follow-up) |
| 8 | `negative-library-admin-screen` | Admin paneli ekranı — hardcoded liste yönetimi UI'a gelir |
| 9 | `local-to-ai-reference-bridge` | Storage bridge / public URL bridge — local diskten AI'ye reference taşıma |
| 10 | `external-source-connector-midjourney` | Midjourney / browser extension / import bridge / source connector |
| 11 | `dpi-resolution-batch-fix` | Quality score'da düşen görselleri batch düzeltme |
| 12 | `local-library-deep-recursion` | Phase 5 root + first-level; daha derin recursion ihtiyacı çıkarsa |
| 13 | `local-library-store-level-override` | Phase 5 user-level; store-level override ihtiyacı çıkarsa |
...
- **AI mode local kaynaklı reference desteklemiyor (R17.2):** Bilinçli kapsam dışı. Local diskten AI'ye köprü için `local-to-ai-reference-bridge` carry-forward (#9).
- **Bulk delete yok:** Tek tek silme Phase 5'te. Toplu silme `bulk-delete-local-assets` carry-forward (#3).
- **Otomatik OCR / background detection yok:** Phase 6 (AI Quality Review) ile gelir — `auto-quality-detection-ocr-bg` carry-forward (#2). Quality score Phase 5'te yalnız DPI + Resolution objektif sinyali; yazı/imza tespiti negatif işareti üzerinden manuel.
- **t2i-with-caption UI akışı yok (R17.1):** z-image provider entegre ama "reference görseli betimle → caption ile t2i" akışı UI'da yok; `caption-then-prompt-flow` carry-forward (#7).
