...
> **Status:** 🟡 **V1 Pending — A-H + G.1 canlı PASS (Pass 16 sonrası), I/J/K/L/M/N/O/P kullanıcı browser smoke'a açık** (HEAD `72522f3`+). Phase 8 V1 implementation complete (33 task, otomasyon gate'leri PASS — TS strict 0, 1743 + 946 test yeşil). Pass 16 (HEAD `72522f3`) gerçek browser ürün QA turunda admin akışı uçtan uca koşturuldu: Apply (Quick Pack default + Render et enabled) → POST /api/mockup/jobs 202 → S7 polling (Pack Hazırlanıyor 0/3 → 3/3) → BullMQ MOCKUP_RENDER worker Sharp local 3/3 SUCCESS → S7 → S8 auto-redirect → S8 result page (cover + 2 grid render canlı thumbnail 800×600) → "Listing'e gönder" handoff → /listings/draft yaratıldı → KIE Gemini 2.5 Flash AI Oluştur title+desc+13 tag canlı + honest submit fail (price missing). **Pass 16 bug fix (commit `72522f3`):** S8ResultView + CoverSwapModal `<img src={render.outputKey}>` storage path direkt src kullanıyordu (broken thumbnail) → `/api/mockup/jobs/[jobId]/renders/[renderId]/download` image stream endpoint'ine geçildi. Phase 8 self-contained (KIE bağımsız Sharp local renderer); runbook 4.2 honest-fail path YOK. Detay: [`./phase8-manual-qa.md`](./phase8-manual-qa.md) "Bulgular — 2026-05-04 (Pass 16)".
...
- ~~S8 result + CoverSwap render thumbnail bug — `<img src={render.outputKey}>` storage path'i doğrudan src'e koyuyordu, browser current page'e relative interpret edip 404'e düşürüyordu; kullanıcı render thumbnail'larını göremiyordu~~ → **kapatıldı** (HEAD `72522f3`, Pass 16): mevcut `/api/mockup/jobs/[jobId]/renders/[renderId]/download` endpoint'i image stream döndürür, preview için doğrudan `<img src>` olarak kullanıldı (auth ownership server-side korunur). Pass 16 gerçek browser ürün QA turunda tespit edildi: admin akışı uçtan uca canlı koşturuldu (Apply → POST /api/mockup/jobs 202 → S7 polling → BullMQ MOCKUP_RENDER worker Sharp local 3/3 SUCCESS → S7 → S8 auto-redirect → fix sonrası 800×600 PNG canlı thumbnail → handoff → KIE generate-meta canlı → honest submit fail "price missing"). 19/19 mevcut S8ResultView + CoverSwapModal UI test PASS (regression yok).
...
## Status: 🟡 Phase 8 V1 Pending — A-H + G.1 canlı PASS (Pass 16 sonrası), I/J/K/L/M/N/O/P kullanıcı browser smoke'a açık
