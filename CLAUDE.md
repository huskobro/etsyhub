...
> Bu entry **özet**tir. Canonical invariant → `docs/claude/mockup-
> studio-zoom-navigator.md` §5/§7. Tarihsel detay → `docs/claude/
> archive/phase-log-97-135.md` (Phase 135 entry).

**Davranış (Fix A — commit `cdb7b10`, pushed):**
Phase 134 sonrası zoom >100%'de (örn. %400) viewfinder/marker
köşelere ulaşamıyordu (pan reach kaybı). Kök neden: `vfCx = 50 −
(ox / fullCompW) × …`; `ox = mediaPos × plateW × PAN_K` zoom'dan
**bağımsız**, `fullCompW = bbox × grp.scale × previewZoom` zoom ile
büyüyor → zoom %400'de `vfCx ≈ 50` (köşe yok). Marker clamp (Phase
134) neden DEĞİLDİ (z400'de tetiklenmiyordu — `mkClamp:false`).
Fix: `.k-studio__media-pos` (orta panel preview) translate
`ox × effectiveZoom` (`effectiveZoom = chromeless ? 1 :
previewZoom` — rail thumb DAİMA 1, bağımsız). `resolveMediaOffsetPx`
(canonical state + Sharp export) **DEĞİŞMEDİ** — zoom export'ta yok
(composition scale=1, pan-reach orada zaten yeterli) → §11.0
Preview = Export korunur. `vfCx` numerator `panOx = ox ×
previewZoom` → cebirsel sadeleşme → **vfCx zoom-bağımsız**
(mediaPos = ±1 her zoom'da köşeye ulaşır; içerik eşleşmesi Phase
129 korunur). `{0,0}` → `0 × zoom = 0` byte-identical no-op.
Browser-kanıtlı: zoom 75/100/160/400 × no-pan + 4 köşe →
vfL≈0%@BR ≈100%@TL her zoom; marker DAİMA panel içinde
(`mkInside:true`); rectangle overflow serbest (Shots.so canonical);
B-consistency 16:9/1:1/4:5/9:16 × Cascade/Tilted/Fan
`contentMatch=true` (Phase 133 shared framing + Phase 134 radius
korundu, yeni divergence YOK). Quality gates: tsc clean,
vitest 739/739, build ✓.

**3 kavram kesin ayrı (Phase 135 dersi):** (1) viewfinder
rectangle overflow = SERBEST, (2) center marker visibility =
CLAMP'li (Phase 134), (3) pan reach / navigable range = TAM
(zoom-aware). Visibility clamp ↔ interaction clamp
karıştırılmaz (karıştırılınca pan reach kaybolur).

**Dokümantasyon refactor:** CLAUDE.md 27,694 satır / 1.33 MB →
authoritative core (~2,560 satır) + router tablosu. Phase
anlatısı (≈24,300 satır / %88) `docs/claude/archive/` altına
"NOT authoritative" damgalı taşındı. Mockup Studio Behavior
Contract (canonical living invariant) + 4 modül-bazlı topic doc
`docs/claude/` altına AUTHORITATIVE damgalı çıkarıldı (bilgi
kaybı YOK). Diğer modüllerin invariant'ları (Canonical Surface
A–Y, Library/Selections/Products boundary, Review Freeze, Marka)
CLAUDE.md core'da KALDI (archive'a taşınmadı). Detay → bu
turun final raporu + archive Phase 135 entry.
